...
y_true = [2, 1, 0, 0, 1, 0]
y_pred = [2, 1, 2, 0, 0, 1]
y_true = cp.asarray(y_true) #Unlike NumPy, Universal Functions in CuPy only work with CuPy array or scalar. They do not accept other objects (e.g., lists or numpy.ndarray)
...
def confusion_matrix(actual, predicted, classes):
    ...
    for i in range(len(classes)):
        for j in range(len(classes)):
            ...
            matrix[i,j] = cp.sum((actual == classes[i]) & (predicted == classes[j])) #np.sum adds all elements in the list
